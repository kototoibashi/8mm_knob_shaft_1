...
module shaft_hole() {
    ...
    translate([0, 0, -1])
        cylinder(d = SHAFT_DIAMETER, h = KNOB_HEIGHT * 0.9 + 1);
}
...
module knob_main() {
    difference() {
        ...
        for (i = [0 : GRIP_FINS - 1]) {
            rotate([0, 0, i * (360 / GRIP_FINS)])
                translate([KNOB_DIAMETER / 2, 0, -1])
                    cylinder(d = 4, h = KNOB_HEIGHT + 2, $fn = 12);
        }

        for (i = [0 : GRIP_FINS2 - 1]) {
            rotate([0, 0, (i + 0.5) * (360 / GRIP_FINS2)])
                translate([KNOB_DIAMETER / 2 + 10, 0, -1])
        ...
        translate([0, 0, -1])
            cylinder(d = KNOB_DIAMETER - WALL_INNER * 2, h = KNOB_HEIGHT - WALL_INNER + 1);
    ...
    intersection() {
        ...
        union() {
            // シャフトボス：シャフト穴周辺の肉付け
            cylinder(d = BOSS_DIAMETER, h = KNOB_HEIGHT * 0.9);
...
// 実行
knob_with_curved_text("<<OPEN  CLOSE>>");